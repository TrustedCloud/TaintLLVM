#ifndef TAINT_TYPE_CHECKING_H
#define TAINT_TYPE_CHECKING_H
...
public:
  ...
  void AddAnnotation(Expr *E, StringRef A) const {
    ...
    if (A.size()) {
      E->setType(CI.getASTContext().getTaintType(E->getType(), A));
    }
  }
  ...
  void RemoveAnnotation(Expr *E) const {
    ...
    if (auto *AT = dyn_cast<TaintType>(T)) {
  ...
  llvm::StringRef AnnotationOf(const Type *T) const {
    ...
    if (auto *AT = llvm::dyn_cast<TaintType>(T)) {
  ...
  void visitCall(CallExpr *E) {
    ...
    if (D) {
      ...
      if (D->param_size() == E->getNumArgs()) {
        ...
        #ifndef NDEBUG
        llvm::errs() << "UNSOUND: varargs function\n";
        #endif
      }
      ...
      #ifndef NDEBUG
      llvm::errs() << "UNSOUND: indirect call\n";
      #endif
    }
  }
